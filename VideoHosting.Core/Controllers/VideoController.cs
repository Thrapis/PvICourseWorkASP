...
using VideoHosting.Core.Models.Utils;
...
namespace VideoHosting.Controllers
{
	public class VideoController : Controller
	{
		...
		public IActionResult Watch([FromQuery(Name = "pageId")] string pageId)
		{
		...
		[AcceptVerbs("Get")]
		[ResponseCache(Duration = 120, Location = ResponseCacheLocation.Any, VaryByHeader = "pageId;quality")]
		public EmptyResult GetVideoSource([FromQuery(Name = "pageId")] string pageId, [FromQuery(Name = "quality")] int quality)
		{
			...
			HttpContext.Response.BodyWriter.WriteAsync(video.Data);
		...
		[AcceptVerbs("Get")]
		[ResponseCache(Duration = 30, Location = ResponseCacheLocation.Any, VaryByHeader = "pageId")]
		public IActionResult GetThumbnail([FromQuery(Name = "pageId")] string pageId)
		{
			//string pageId = Request.QueryString.Get("pageId");
            ...
            try
            {
				...
				Response.BodyWriter.WriteAsync(preview.Data);
				//Response.End();
		...
		[Route("addTagPost")]
		public IActionResult AddTag(string pageId, string tagName)
		{
			if (User.Identity.IsAuthenticated)
            {
				var connection = MainOracleConnection.GetConnection();
		...
		[Route("removeTagPost")]
		public IActionResult RemoveTag(string pageId, string tagName)
		{
			if (User.Identity.IsAuthenticated)
			{
				var connection = MainOracleConnection.GetConnection();
		...
		public IActionResult UpdateVideoName(string pageId, string videoName)
		{
			if (User.Identity.IsAuthenticated)
			{
				var connection = MainOracleConnection.GetConnection();
		...
		public async Task<IActionResult> UpdateThumbnail(string pageId, IFormFile imageFile)
		{
			if (User.Identity.IsAuthenticated)
			{
				var connection = MainOracleConnection.GetConnection();
				...
				string directory = MyServer.MapPath("~/Upload/TempUpload/");
				if (!Directory.Exists(directory)) Directory.CreateDirectory(directory);

				string uploadedPath = MyServer.MapPath("~/Upload/TempUpload/" + pageId + FilePathHelper.GetExtension(imageFile.FileName));
				
				//imageFile.SaveAs(uploadedPath);

				await SaveFormFile(imageFile, uploadedPath);
		...
		public IActionResult RemoveVideo(string pageId)
		{
			if (User.Identity.IsAuthenticated)
			{
                var connection = MainOracleConnection.GetConnection();
		...
		[Route("uploadPost")]
		public async Task<ContentResult> UploadVideo(IFormFile videoFile, string videoName, string[] tags)
		{
			if (User.Identity.IsAuthenticated && videoFile != null)
			{
				...
				string uploadedPath = MyServer.MapPath("~/Upload/TempUpload/" + stringGuid
					+ FilePathHelper.GetExtension(videoFile.FileName));

				await SaveFormFile(videoFile, uploadedPath);
				...
				await Task.Factory.StartNew(() => HandleVideo(stringGuid, account.Id, videoName, tags, uploadedPath));
		...
		void HandleVideo(string stringGuid, int accountId, string videoName, string[] tags, string uploadedPath)
        {
			...
			string thumbnailPath = MyServer.MapPath("~/Upload/TempUpload/" + stringGuid + "_th.jpeg");
			string convertedPath_480 = MyServer.MapPath("~/Upload/TempUpload/" + stringGuid + "_480.mp4");
			string convertedPath_720 = MyServer.MapPath("~/Upload/TempUpload/" + stringGuid + "_720.mp4");
			string convertedPath_1080 = MyServer.MapPath("~/Upload/TempUpload/" + stringGuid + "_1080.mp4");
